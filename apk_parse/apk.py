...
from . import androconf, bytecode
...
class APK(object):
    ...
    def process(self):
        ...
        if self.zipmodule == 0:
            ...
            zip_input = io.BytesIO(self.__raw) if not self.as_file_name else self.filename
        ...
        if not self.android_manifest_present and not self.cert_present and self.inner_apk is not None:
            ...
            sub_zip_input = io.StringIO(sub_apk_input) if not sub_apk_is_filename else sub_apk_input
    ...
    def show(self):
        ...
        for i in self.get_files():
            try:
                print("\t%s %s %x" % (i, self.files[i], self.files_crc32[i]))
            except KeyError:
                print("\t%s %s" % (i, self.files_crc32[i]))

        print("PERMISSIONS: ")
        details_permissions = self.get_details_permissions()
        for i in details_permissions:
            print("\t %s %s" %  (i, details_permissions[i]))
...
class StringBlock(object):
    ...
    def decode2(self, array, offset, length):
        ...
        return data.encode().decode("utf-8", 'replace')
